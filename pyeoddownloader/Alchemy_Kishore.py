...
def getyestbusiness(nse_fulldate) :
	ts = pandas.read_sql('SELECT max(nse_date) from eod_data', con=engine)
	x = ts.loc[0, 'max(nse_date)'] 
	return x


def getyestbusinessold(nse_fulldate):
...
def execute():
	...
	#=================================================================================== Downloader ===============================================================================================================================#
		...
		filedownloaded = nse_downloader (fut_cmboi_link,nse_raw_path,str(fut_cmboi_link).split('/')[-1])						# Arguments = Future Bhavcopy Link, File Download Path, Filename (These Files Contain Addtional .zip as extn)
