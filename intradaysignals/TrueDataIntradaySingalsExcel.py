...
symbol_list = ["GRASIM","CONCOR","ARVIND","RAMCOCEM","BHARATFORG","JETAIRWAYS","BPCL","VEDL","ASIANPAINT","REPCOHOME","CENTURYTEX","PFC"]
sindex = 0
gotresponse = 0;
...
class TrueDataExternalEventsSink:
    def OnBarData(self, RequestId, unknown):
        ...
        oi = z['oi'].iloc[-1]
        time1 = z.index[-1].strftime('%H:%M:%S')

        # print(symbol , dt1, opn, high, low,close, volume, oi)
        
        if len(z.index) > 15:
            
            z = z.tail(15)
            prevminvolume = z['volume'][13]
            
            
            if ((prevminvolume  >= z['volume'].max())):
                print("Highest Volume found for ---------", symbol, time1)
        ...
        if len(x.index) > 20:
            ...
            if len(highs) > 3 or len(lows) > 3:
                if (higher(x.iloc[highs]) and higher(x.iloc[lows])):
                    print(
                        "higher highs or higher lows found for Long -------", symbol, time1)
                    msgs[symbol] = "higher highs or higher lows found for LONG"+time1    
                    pahhsig = 1
                if (lower(x.iloc[highs]) and lower(x.iloc[lows])):
                    print(
                        "lower highs or lower lows found for short--------", symbol, time1)
        ...
        if len(y.index) > 30:
            if (((close / y['low'].min()) > 0.999) and ((close / y['low'].min()) < 1.001)):
                print(
                    "Stock near days Low watch out ----------- ", symbol, time1)
                msgs[symbol] = "near days LOW watch out"+time1   
                daylowsig = 1
            if ((close / y['high'].max() > 0.999) and (close / y['high'].max() < 1.001)):
                print(
                    "Stock near days High watch out ----------", symbol, time1)
                msgs[symbol] = "near days HIGH watch out"+time1 
                dayhighsig = 1
        
        
        # df = pd.DataFrame(parsed_data)
        signals.append((symbol,time1, opn, high, low, close, volume,volsig, pallsig, pahhsig, daylowsig, dayhighsig,supersig, cci34,superTrend, msgs.get(symbol,"")));
